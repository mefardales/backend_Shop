...
class UserTestCase(APITestCase):
    ...
    def test_get_user(self):
        user = UserFactory()
        url = reverse('list', kwargs={'id': user.id})
    ...
    def test_get_user_empty(self):
        user = UserFactory()
        url = reverse('list', kwargs={'id': user.id + 1})
        response = self.client.get(f'{url}', format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_create_user(self):
        user_dict = self.create_user_data()
        response = self.client.post(reverse('create'), user_dict, format='json')
